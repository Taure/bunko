...
start() ->
    ...
    application:set_env(bunko, bunko_test_repo, #{
        pool => bunko_test_repo,
        pool_module => kura_pool_pgo,
        driver_module => kura_driver_pgo,
        database => env(~"BUNKO_TEST_DB", ~"bunko_test"),
        hostname => env(~"BUNKO_TEST_DB_HOST", ~"localhost"),
        port => env_int(~"BUNKO_TEST_DB_PORT", 5557),
        username => env(~"BUNKO_TEST_DB_USER", ~"postgres"),
        password => env(~"BUNKO_TEST_DB_PASSWORD", ~"root"),
        pool_size => 5
    }),
